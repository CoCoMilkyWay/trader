...
class AdaptiveSuperTrend:
    ...
    def __init__(self, atr_len: int = 50, factor: float = 3, dump_ind: bool = False):
        ...
        # Fixed size of 3 (high, medium, low) volatility
        ...
        if self.dump_ind:
            ...
            self.long_trades = {'returns': 0.0,
                                'wins': 0, 'total': 0, 'hold_time': 0}
            self.short_trades = {'returns': 0.0,
                                 'wins': 0, 'total': 0, 'hold_time': 0}
    ...
    def update(self, high: float, low: float, close: float, ts: float) -> Tuple[bool, bool]:
        ...
        if self.is_initialized:
            self.prev_atr = (tr * self.ema_multiplier) + \
                (self.prev_atr * (1 - self.ema_multiplier))

            self.prev_atr = (
                self.prev_atr * (self.atr_len - 1) + tr) / self.atr_len
        else:
            self.prev_atr = tr
        ...
        adaptive_factor = self.factor * \
            self.cluster_factors[self.current_cluster]
        ...
        final_upper = upper if (
            upper < self.prev_upper or close > self.prev_upper) else self.prev_upper
        final_lower = lower if (
            lower > self.prev_lower or close < self.prev_lower) else self.prev_lower
        ...
        if direction != self.prev_direction:
            ...
            if self.centroids[1] != 0:
                self.cluster_factors = [
                    self.centroids[0]/self.centroids[1], 1, self.centroids[2]/self.centroids[1]]
            ...
            if self.dump_ind:
                ...
                if self.trade_start_price != 0:
                    trade_result = (close - self.trade_start_price) * \
                        self.prev_direction / self.trade_start_price
                    ...
                    if True:  # trades['total'] == 1:
                        ...
                        self.avg_return[idx] = (
                            (trades['returns'] / trades['total'] * 100))
                        self.avg_win_rate[idx] = (
                            (trades['wins'] / trades['total'] * 100))
                        # Convert seconds to hours
                        self.avg_hold[idx] = (
                            trades['hold_time'] / trades['total'] / 3600)
                        # trades['total'] = 0
                        # trades['wins'] = 0
                        # trades['hold_time'] = 0
    ...
    def get_stats(self, code: str):
        ...
        print(
            f"Long trades:  Return: {self.avg_return[0]:6.2f}% | Win Rate: {self.avg_win_rate[0]:5.1f}% | Avg Hold: {self.avg_hold[0]:5.1f}h")
        print(
            f"Short trades: Return: {self.avg_return[1]:6.2f}% | Win Rate: {self.avg_win_rate[1]:5.1f}% | Avg Hold: {self.avg_hold[1]:5.1f}h")
