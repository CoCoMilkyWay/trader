...
def _lxr_fundamental_file(path: str, wt_asset: Dict, wt_tradedays: Dict):
    ...
    #     for pd in pds:
    #         if not str(pd).endswith('parquet'):
    #             continue
    #
    #         # # SSE.STK.603233_2017.parquet
    #         # exchange, product, rest = pd.split(".")[:3]
    #         # code, year = rest.split("_")
    #
...
# def _lxr_fundamental_file(path: str, wt_asset: Dict, wt_tradedays: Dict):
#     state = _check_state(path, "LXR-Fundamental", 1)
#     exgs = cfg_stk.exchg
#     API_LIMITS = 10
#     N = 3
#     if state != 0:  # exist
#         old_lxr = load_sparse_df(path)
#     else:
#         old_lxr = pd.DataFrame([])
#
#     codes = []
#     for exg in exgs:
...
#             codes.append(key)
#     dates = pd.to_datetime(wt_tradedays["CHINA"], format='%Y%m%d').sort_values()
#     all_metrics:List[str] = list(LXR_API.all_metrics)
#     all_dates:List[datetime] = [date for date in dates if date < datetime.today()]
#     all_codes:List[str] = codes
#     index = pd.MultiIndex.from_product([all_metrics, all_dates, all_codes],
#                                        names=['metric', 'date', 'code'])
#     DATA = pd.DataFrame(np.nan, index=index, columns=['value'], dtype=np.float32)
#     print(f"fundamental data: {DATA.memory_usage(deep=True).sum()/1000/1000:.2f} MBs")
#     print(DATA.shape)
#
#     print(DATA)
#     assert False
...
#                 ipoTime = datetime.fromisoformat(ipoDate)
#                 ranges = _generate_dt_ranges(
#                     ipoTime.year, curTime.year, API_LIMITS)
#                 for range in ranges:
#                     r = LXR_API.query_fundamental(
#                         fsTableType, range[0], range[1], [pending_asset])
#                 print(r)
#                 print(type(r))
#                 print(ranges)
...
def generate_database_files(wt_assets: list, force_sync: bool = False):
    ...
    processed_asset_list = os.listdir(mkdir(f"{cfg_stk.STOCK_DB_BAR_DIR}/"))
...
def store_bars(df, file_path):  # 'date' 'time' 'open' 'high' 'low' 'close' 'vol'
    if os.path.exists(file_path):
        ...
        dtHelper.store_bars(barFile=store_path,
                            firstBar=buffer, count=len(df), period="m1")
#
# # ================================================
