...
# FUTURES_SYMBOLS=(BTCUSDT ETHUSDT BNBUSDT DOGEUSDT SHIBUSDT ADAUSDT) #Add more symbols as needed
readarray -t FUTURES_SYMBOLS < <(sed 's/\r$//' "${SYMBOL_TABLE_DIR}/um_symbols.txt")
# BREAKPOINT_SYMBOL="INJUSDT"
# start_idx=$(printf '%s\n' "${FUTURES_SYMBOLS[@]}" | grep -n "${BREAKPOINT_SYMBOL}" | cut -d: -f1)
# start_idx=$((start_idx - 1))  # Convert to 0-based index

# Return subarray from target onwards
FUTURES_SYMBOLS=("${FUTURES_SYMBOLS[@]:$start_idx}")


echo "Symbols loaded: ${FUTURES_SYMBOLS[@]}"
