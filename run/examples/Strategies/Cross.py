...
class CrossStra(BaseCtaStrategy):
    
    def __init__(self, name:str, code1:str, code2:str, barCnt:int, period:str, capital:float,margin_rate:float,money_pct:float,isForStk:bool = False):
    ...
    def on_calculate(self, context:CtaContext):
        ...
        df_diff_price = df_bars2.closes - df_bars1.closes
        ...
        context.stra_log_text('当前净值:%.2f,当前仓位：%.2f  %.2f' \
                              %(1 + context.stra_get_fund_data(0)/capital,curPos1,curPos2))

        if curPos1 == 0 and curPos2 ==0: #空仓情况下
            if new_diff_price > up_price:
                self.cur_money = capital + context.stra_get_fund_data(0)
                context.stra_enter_long(code2, math.floor(self.cur_money * money_pct *margin_rate/ trdUnit_price2),
                                                        'enterlong')
                context.stra_enter_short(code1, math.floor(self.cur_money * money_pct *margin_rate/ trdUnit_price1),
                                                        'entershort')
                context.stra_log_text("价差突破%.2f高点，做多价差组合" % (up_price))

            elif new_diff_price < down_price:
                self.cur_money = capital + context.stra_get_fund_data(0)
                context.stra_enter_long(code1, math.floor(self.cur_money * money_pct * margin_rate / trdUnit_price1),
                                        'enterlong')
                context.stra_enter_short(code2, math.floor(self.cur_money * money_pct * margin_rate / trdUnit_price2),
                                         'entershort')
                context.stra_log_text("价差突破%.2f低点，做空价差组合" % (up_price))

        else:
            if curPos2 > 0 and new_diff_price < stop_up_price:
                context.stra_set_position(code2, 0, 'clear')
                context.stra_set_position(code1, 0, 'clear')
                context.stra_log_text("价差多头组合反向突破，平仓")
            elif curPos1 > 0 and new_diff_price > stop_down_price:
                context.stra_set_position(code1, 0, 'clear')
                context.stra_set_position(code2, 0, 'clear')
