...
from sklearn.utils.validation import check_X_y, check_array, validate_data, _check_sample_weight # type: ignore
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    for i in range(n_programs):
        ...
        program.parents = genome # type: ignore
        ...
        # print(f"Evaluating: {str(program):<80} | Fitness: {program.raw_fitness_:>10.6f}", flush=True)
        if max_samples < n_samples:
            # Calculate OOB fitness
            program.oob_fitness_ = program.raw_fitness(X, y, oob_sample_weight) # type: ignore
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def __sklearn_tags__(cls):
        ...
        -------
        tags : dict
            Dictionary of tags describing estimator properties.
        """
        return {
            'allow_nan': False,
    ...
    def _estimator_type(self):
        """Return type of estimator."""
        return "" # "regressor" or "classifier" for SymbolicClassifier
    ...
    def _validate_data(self, X, y=None, *, y_numeric=False, **kwargs):
        ...
        y_numeric : bool, default=False
            Whether to ensure that y is numeric.
        **kwargs : dict
            Additional arguments to be passed to validate_data.
        """
        check_params = {
            'accept_sparse': False,
            'dtype': 'numeric',
        ...
        if y is None:
            ...
            # Remove y_numeric from kwargs if present as it's not for X validation
            check_params.pop('y_numeric', None)
            X = check_array(X, **check_params)
            y = check_array(y, ensure_2d=False, dtype='numeric' if y_numeric else None)
    ...
    def _verbose_reporter(self, run_details=None):
        ...
        if run_details is None:
            ...
            print(line_format.format('Gen', 'Length', 'Fitness', 'Length',
                                    'Fitness', 'OOB Fitness', 'Time Left', 'Formula'))
            print('-' * 130)
        else:
            # Get current generation's programs
            current_gen = self._programs[-1]
            # Get their fitness values
            fitness_values = [program.raw_fitness_ for program in current_gen]
            ...
            best_program = current_gen[best_idx]
            best_program_str = str(best_program)

            # Truncate long formulas with ellipsis
            if len(best_program_str) > 37:
                best_program_str = best_program_str[:37] + "..."
            ...
            if self.max_samples < 1.0:
                ...
                line_format = '{:4d} {:8.2f} {:16g} {:8d} {:16g} {:16g} {:<12} {:<40}'
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for gen in range(prior_generations, self.generations):
            ...
            population = list(itertools.chain.from_iterable(population)) # type: ignore
        ...
        if isinstance(self, TransformerMixin):
            ...
            components = list(range(self.hall_of_fame)) # type: ignore
            indices = list(range(self.hall_of_fame)) # type: ignore
            # Iteratively remove least fit individual of most correlated pair
            while len(components) > self.n_components: # type: ignore
                ...
                indices.remove(worst) # type: ignore
