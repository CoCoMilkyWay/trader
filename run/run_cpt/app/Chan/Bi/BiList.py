from typing import List, Optional, Union, overload, Callable
...
[First Pass Check]──────────────────────┐
   |                                    |
   v                                    v
Has existing Bi list?          Add to free_klc_lst
   |                                    |
   v                                    v
Check FX Type                Check for possible match
   ...
   |     |
   |     v
   |  Compare with last_end FX
   |     |
   |     ├─── Same FX ──────┐
   |     |                  v
   |     |            Try Update End Point
   |     |                  |
   |     |                  ├─── Success ──> Update last_end
   |     |                  |
   |     |                  └─── Fail ────> Try Peak Update
   |     |                                      |
   |     v                                      v
   |  Different FX                         Peak Update Check
   |     |                                      |
   |     v                                      |
   |  Can Make New Bi? ◄──── NO ─────────────────┘
   |     |                                      ^
   ...
   └─> Try Add Virtual Bi
         |
         ├─── Check Extension of Current End
         |        |
         |        └─── Possible ──> Update Virtual End
         |
         ├─── Check New Virtual Bi Formation
         |        |
         |        └─── Valid ────> Create Virtual Bi
         |
         └─── Check Virtual Peak Update
                  |
                  └─── Valid ────> Update Peak

[Bi Creation Rules Detail]
   |
   ├─── Check Bi Span
   |     ├─── Strict Mode: >= 4 K-lines
   |     └─── Normal Mode: >= 3 K-lines & Unit Count >= 3
   |
   ├─── Validate FX Conditions
   |     ├─── Check Direction Validity
   |     ├─── Check Price Levels
   |     └─── Check Gap Conditions
   |
   ├─── Check End Peak Requirements
   |     ├─── For Bottom: No Higher High Between
   |     └─── For Top: No Lower Low Between
   |
   └─── Additional Configuration Checks
         ├─── Allow Sub-peak Option
         ├─── Gap Handling Rules
         └─── Bi Algorithm Selection
