...
class PA_Volume_Profile():
    def __init__(self):
        ...
        # simplify n-day/bi data structure
        # self.n_day_volume_profile: List[deque[List[int]]] = [
        #     deque() for _ in range(self.N_day)]
        self.n_bi_volume_profile: List[deque[List[int]]] = [
            deque() for _ in range(self.N_bi)]
    ...
    def update_volume_profile(self, batch_volume_profile: List, type: str) -> None | List[Union[List[float], List[int]]]:
        ...
        #     self.n_day_volume_profile[self.list_day_idx].extend(
        #         [[0, 0] for _ in range(_len)])
        ...
        if new_bi and self.volume_inited:
            ...
            bi_volume_profile_total = [
                bi_volume_profile[i][0] + bi_volume_profile[i][1] for i in range(_len)]
            self.n_bi_volume_profile[self.list_bi_idx].clear()
            self.n_bi_volume_profile[self.list_bi_idx].extend(
                [[0, 0] for _ in range(_len)])
        else:
        ...
        if bi_volume_profile_total:
            price_mapped_volume: None | List[Union[List[float], List[int]]] = [
            ...
            if left_cnt is not None and right_cnt is not None:
                ...
                price_mapped_volume[0] = [round((i+self.volume_idx_min) * self.price_bin_width, 2)
                                          # price
                                          for i in range(left_index, right_index+1)]
                # volume
                price_mapped_volume[1] = bi_volume_profile_total[left_index:right_index+1]
        else:
