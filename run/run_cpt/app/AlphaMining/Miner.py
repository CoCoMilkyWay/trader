...
from Mining.RL.Agent.Agent_MCTS_MDP import Agent_MCTS_MDP
...
class Miner:
    def __init__(self):
        ...
        self.agent = Agent_MCTS_MDP(self.env)
        self.agent.run()

    def test_basic(self):
        """ tets basic parser, evaluate, alpha-pool functions"""
        with open(os.path.join(os.path.dirname(__file__), "Mining/Data/report.json")) as f:
            report = json.load(f)
