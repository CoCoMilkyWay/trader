...
class StockData:
    ...
    def _load_exprs(self, exprs: Union[str, List[str]]) -> pd.DataFrame:
        ...
        print('config: ', self._start_time, self._end_time, end_index-start_index-1)
        print('real: ', real_start_time, real_end_time, self.max_backtrack_days, self.max_future_days, end_index + self.max_future_days - start_index + self.max_backtrack_days-1)
    ...
    def _get_data(self) -> Tuple[torch.Tensor, pd.Index, pd.Index]:
        ...
        # First check and handle completely empty columns
        empty_cols = df.columns[df.isna().all()]
        if not empty_cols.empty:
            print(f"Warning: Found completely empty columns: {empty_cols}")
            # For completely empty columns, fill with 0 since ffill won't work
            df[empty_cols] = 0.0
        # Forward fill, then backward fill to handle leading NaNs
        df = df.ffill()
        # df = df.bfill()
        # If there are still any NaNs (if a column has all NaNs in some stocks), fill with 0
        df = df.fillna(0)
        ...
        values = df.values
        print('values(before reshape): ', values.shape)
        ...
        print('values(final): ', values.shape)
