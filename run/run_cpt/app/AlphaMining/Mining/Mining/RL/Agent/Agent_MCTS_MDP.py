...
class Agent_MCTS_MDP:
    ...
    def __init__(self, env: ENV):  # gym style standard environment
        # Box(low=0, high=140, shape=(15,), uint8)
        # self.obv_space = env.observation_space
        # self.act_space = env.action_space  # Discrete(140)
        # self.state_size = int(self.obv_space.shape[0]) # type: ignore
        # self.action_size = int(self.act_space.n)  # type: ignore
        #
        ...
        # self.game = Game(env)
        from Mining.RL.Agent.Core.Game import Game_CartPole
        self.game = Game_CartPole()
        self.replay_buffer = {}

    def run(self):
        ray.init(
            # runtime_env = {"working_dir": os.path.join(os.path.dirname(__file__), "..")}
            )
        ...
        self.replay_buffer_worker = ReplayBuffer.options(num_cpus=1).\
            remote(self.checkpoint, self.replay_buffer)
        # self.reanalyse_worker = Reanalyse.options(num_cpus=1).\
        #     remote(self.checkpoint)
        self.playing_workers = [
            PlayLoop.options(num_cpus=1).
            remote(self.checkpoint, self.game)
