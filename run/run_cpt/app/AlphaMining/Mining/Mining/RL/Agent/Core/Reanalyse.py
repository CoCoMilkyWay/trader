...
class Reanalyse:
    ...
    def reanalyse(self, replay_buffer, checkpoint):
        ...
        while ray.get(
            ...
            game_id, trajectories, _ = ray.get(
            ...
            if use_last_net_value:
                observations = numpy.array(
                    [
                        trajectories.get_stacked_observations(
                            i,
                            stacked_observations,
                            len(action_space),
                        )
                        for i in range(len(trajectories.values))
                    ]
                )
                ...
                trajectories.reanalysed_predicted_root_values = (
                    torch.squeeze(values).detach().cpu().numpy()
                )

            replay_buffer.update_trajectories.remote(game_id, trajectories)
