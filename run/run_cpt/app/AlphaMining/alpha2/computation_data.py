...
finish_action = tuple([finish_token_idx, null_token_idx,
                      null_token_idx, null_token_idx])

all_action_list = [start_action, finish_action] + \
    unary_actions + binary_actions + ternary_actions
start_action_idx = 0
finish_action_idx = 1
NUM_ACTIONS = len(all_action_list)


register_token_ids = [i for i in range(
    NUM_OPERANDS) if isinstance(operand_tokens[i], RegisterToken)]
