...
class TechnicalAnalysis:
    ...
    def __init__(self, code:str, train:bool = False):
        ...
        self.lv_list = [lv[0] for lv in self.config.lv_list] # [..., 1min]
        self.levels = tuple(reversed(self.lv_list)) # [1min, ...]
        ...
        self.chan_snapshot: CChan = CChan(
            code=code,
        ...
        self.aroon          = aroon(self.highs[i], self.lows[i], 25)
        ...
        self.feature_specs = {
            ...
            'aroon': {'instance': self.aroon, 'features': [('aroon_up', -1), ('aroon_down', -1)], 'Scaler': ScalingMethod.STANDARD,},
        ...
        self.current_row = 0
        if self._train:
            self.features_history = np.zeros((100_000, self.n_cols), dtype=np.float32) # 3600 * 24 = 86400
        else:
    ...
    def update_features_and_labels(self):
        ...
        if self._train:
            self.ts_label.update(self.timestamp[-1], self.closes[0][-1], self.atr10.atr[-1], s_long_switch, s_short_switch)
