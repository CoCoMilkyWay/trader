...
class Parallel_Process:
    def __init__(self, code_info: Dict[str, Dict], Process_Core):
        ...
        set_cpu_affinity(0)
        print("[Main] process assigned to CPU 0...")
        
        # Determine the number of workers based on the available logical CPUs
        self.num_workers = min(logical_cpus - 1, len(self.code_info))
        ...
        print(f"Initializing {self.num_workers} workers...")
        ...
        for i in range(self.num_workers):
            ...
            p = Process(
                ...
                args=(i, self.worker_codes[i], shared_mem, self.shared_control, ring_buffer, lock, Process_Core),
                name=f"Worker-{i + 1}",
                daemon=True
            )
            p.start()
    ...
    def parallel_exec(worker_id:int, worker_codes:list[str], shared_mem, shared_control, ring_buffer, lock, Process_Core):
        ...
        worker_id:  NaN       0     1     2
        """
        set_cpu_affinity(worker_id + 1)  # Pin each worker to a specific core
