...
class TimeSeriesAnalysis:
    def __init__(self, code: str, code_idx: int, shared_tensor: torch.Tensor, plot: bool = False):
        ...
        self.AdaptiveSuperTrend = AdaptiveSuperTrend(
            atr_len=60, factor=10)  # 1 hours
    ...
    def _init_indicators(self):
        ...
        self.indicator_manager.process_definitions(
            TimeSeriesAnalysis_Rules.indicator_definitions)

    def _init_features_labels_and_scalers(self):
        # Generate column names and scaling methods from feature specs
        self.n_features = sum(
            len(spec['features']) for spec in self.indicator_manager.feature_specs.values())
        ...
        for indicator_name, spec in self.indicator_manager.feature_specs.items():
            ...
            for attr_name, idx, attr_type in spec['features']:
                ...
                self.feature_map.append(
                    (spec['instance'], attr_name, idx if idx else None))
                self.scaling_methods.append(scaler)

        # Create label column names
        self.label_names: List[str] = [
            f'label_{i+1}' for i in range(self.n_labels)]

        # Create template for fast instance
        self.result_list = [0.0 for _ in range(
