...
def run_bt():
    ...
    stock_info = json.loads(open(f"{dir}/stock_info.json", encoding='utf-8').read())
    daily_holdings = json.loads(open(f"{dir}/daily_holdings.json", encoding='utf-8').read())
    print(stock_info.keys())
    
    wt_asset = prepare_all_files(stock_info.keys())
