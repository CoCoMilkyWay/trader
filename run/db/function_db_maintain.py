...
def store_bars(df, file_path): # 'date' 'time' 'open' 'high' 'low' 'close' 'vol'
    if os.path.exists(file_path):
        ...
        df['time'] = (df['date']-19900000)*10000 + df['time'].apply(lambda x: int(x.strftime('%H%M')))
...
def process_dataframe(file_path, asset_dict, daily_k_bar):
    def process_juejin():
        ...
        df.columns = ['trade_time', 'open', 'high', 'low', 'close', 'vol', 'amount']
    ...
    def process_tdx():
        ...
        df.columns = ['date', 'time', 'open', 'high', 'low', 'close', 'vol', 'amount']
    ...
    def process_other():
        ...
        df.columns = ['index', 'datetime', 'code', 'open', 'high', 'low', 'close', 'vol', 'amount']
    ...
    if cfg.CHECK_5:
        ...
        if os.path.exists(file_path):
            ...
            if daily_k_bar.shape[0] > 100:
                ...
                daily_k_bar.columns = ['datetime', 'open', 'high', 'low', 'close', 'vol', 'amount']
